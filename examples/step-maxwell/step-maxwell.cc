...
namespace Step23
{
  ...
  double get_amplitude(double time)
  {
    ...
    // if (time < total - delay) return 1.0;
    // if (time < total)
    //   return  std::exp(-pow2((time-total+delay)/width));
    // return 0.0;
    // return 1.0;
  }
  template <int dim>
  double get_shape(const Point<dim> &p)
  {
                                      ...
                                      const unsigned int component) const
  {
    ...
    if (
      ...
      return get_shape(p)*get_amplitude(this->get_time())*std::sin (this->get_time() * 4 * numbers::PI);
    else
                                      ...
                                      const unsigned int component) const
  {
    ...
    if (
      ...
      return get_shape(p)*get_amplitude(this->get_time())*(std::cos (this->get_time() * 4 * numbers::PI) *
              4 * numbers::PI);
    else
      return 0;
  }
  ...
  void WaveEquation<dim>::setup_system ()
  {
    ...
    GridGenerator::subdivided_hyper_rectangle (triangulation, repetitions, left, right);
    int refine_num = 2;
    ...
    for (timestep_number=1, time=time_step;
         ...
         time+=time_step, ++timestep_number)
      {
        std::cout << "Time step " << timestep_number
                  << " at t=" << time << " amp =" <<get_amplitude(time) 
                  << std::endl;
